...
정적 **`index.html`** 은 **검증 전용**입니다. 사용자는 **검증 토큰**을 입력하거나 `/v/{토큰}`·`?id=` 링크로 조회합니다. 미디어 해시 제출·운영 지갑·머클 배치 봉인은 **백엔드(`server/`)·앱 API**에서 처리합니다. 로컬은 `npx serve .` + 서버 `npm run dev`면 기본 `http://localhost:4000`을 씁니다. GitHub Pages는 **`VERITY_PAGES_API`** 또는 **`?api=`** 로 백엔드를 지정합니다.
